...
def loadWordList(wordlist_file, ext):#load pages to check from dictionary
    try:
        ...
        sys.exit("No wordlist file found! Exiting.....")
...
def main(domain, progress=0, ext="a", strict=False, save=True, visible=True, wordlist_file="admin_login.txt"):
    ...
    print("Loading possible admin panel list from file...")
    ...
    if found:
        if save:#save results to a text file
            ...
            print("Results are saved to admin_sniffer_results.txt...")

        print("I found the following results: " + "  ".join(found) + " total progress: %s" % progress)
...
def getRobotsFile(domain):
    print("Trying to get robots.txt file...")
    found = []
    domain = adjustDomainName(domain)#correct domain name for urllib
    ...
    try:
        ...
        for element in data.split("\n"):
            if element.startswith("Disallow:"):
                ...
                print("Disallow rule found in robots.txt for: %s" % (panel_page))
                found.append(panel_page)
        if found:
            print("Some admin panels are found... Saving them to file...")
            saveResults("admin_sniffer_results.txt", found, 0)
            print("Mission Completed...")
        else:
            print("No Admin panel pages are found in robots.txt file")
    except:
        sys.exit("Robots.txt File Not Found on this server!")

if __name__ == "__main__":
    print("        +++++++++++++++++Admin Panel Scanner By Storm29+++++++++++++++++++++++")
    print("        +                It should work with python 2.0 or 3.0               +")
    print("        +                I am not responsible for malicious use!             +")
    print("        ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++\n\n")
